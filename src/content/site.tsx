    ...
    sections: [
      {
        ...
        body: (
          <>
            4DGS World Lab connects video generation and 3D Gaussian
            Splatting: a panoramic video generated by Seedance 2 is split into
            120 frames, each frame is reconstructed into a 3D Gaussian splat
            with World Labs&apos; Marble, and the splats are played back in
            sequence — producing a dynamic, explorable &quot;4DGS&quot; scene
            that runs in a browser tab, no training required.
          </>
        ),
      },
      {
        heading: "A pipeline of specialists",
        body: (
          <>
            No single model can do this today. Panoramic footage is rare in
            video training data, so Seedance can&apos;t hold a valid
            equirectangular projection on its own, and GPT Image 2&apos;s
            &quot;panoramas&quot; don&apos;t wrap at the edges. So each model
            does only what it&apos;s best at: Marble generates a geometrically
            correct first panorama, GPT Image 2 — which does have the
            multimodal context understanding Marble lacks — edits it into a
            layout-matched last frame, and Seedance interpolates the motion
            between the two keyframes. Since Seedance doesn&apos;t accept 2:1
            media, both keyframes are squeezed (not cropped) to 16:9 and the
            output video is stretched back to 2:1, preserving the layout.
          </>
        ),
      },
      {
        heading: "From video to 120 splats",
        body: (
          <>
            The 5-second, 24 fps video yields 120 frames, each submitted to
            the Marble 1.1 API as an independent panorama-to-splat job — far
            faster than working through Marble Studio by hand. A fixed random
            seed across all 120 generations is the only consistency lever
            available when every frame is reconstructed with no knowledge of
            its neighbors.
          </>
        ),
        images: [
      ...
      {
        heading: "Web playback and limitations",
        body: (
          <>
            The viewer is built on Three.js and Spark, remounting one splat
            per tick — up to 24 splat switches per second — with WASD fly
            controls. Because the full sequence is ~864 MB, it defaults to a
            sparse 6-frame preview (~43 MB) covering the same 5-second
            timeline, with opt-in 24/60/120-frame tiers and device detection
            that locks heavy tiers on phones. The remaining limits are
            inherent to per-frame reconstruction: playback is brute force, and
            independent generations shimmer locally even with a fixed seed —
            both pointing toward temporally-aware reconstruction as the next
            step.
          </>
        ),
      },
