...
│  ├─ ParticleBackground.tsx  # 编排:Canvas + 相机/指针 + 形变循环
│  ├─ timeline.ts             # 锚点测量 + 纯函数布局(layoutTimeline 可单测)
│  ├─ sampling.ts             # 点库加载(.bin,带防 SPA-fallback 校验)/缩放/explode
...
| 粒子数量、颜色、大小、亮度、摆动幅度/周期 | `src/config.ts` → `PARTICLES`(`brightness`/`brightnessMobile` 颜色倍增;`swingAmplitude`/`swingPeriod`,正弦摆动) |
...
| 单个形状的旋转/位置/大小微调 | `SHAPE_KEYFRAMES` 条目的 `rotateAxis`+`rotateAngle` / `offset` / `scale`;窄屏(<768px)单独覆盖用 `offsetMobile` / `scaleMobile`,拖动窗口实时生效 |
...
| 鼠标悬浮斥力、相机视差晃动 | `src/config.ts` → `POINTER_FX`(仅桌面指针设备启用,reduced-motion 关闭) |
...
Lenis(平滑滚动) ──→ ScrollContext.progress (ref, 0→1)
                              │ 每帧读取(不触发 React 渲染)
                              ▼
Particles.useFrame: damp 阻尼追踪 → 按 timeline.ts 解析出的 stops 定位区间
                              ▼
   相邻两关键帧逐粒子 lerp(旋转/位移/缩放全部在外层对象逐帧插值,点库共享不复制)
                              ▼
